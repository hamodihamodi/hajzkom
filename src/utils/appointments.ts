...
export function getAppointmentsForBusiness(
  businessId: string,
  locationId?: string,
  locationName?: string,
): Appointment[] {
...
function seedForBusiness(businessId: string, locationId?: string, locationName?: string): Appointment[] {
  const lid = locationId ?? 'loc-1'
  const lname = locationName ?? 'الفرع الرئيسي'
  return [
    {
      id: makeId(), businessId, locationId: lid, locationName: lname,
